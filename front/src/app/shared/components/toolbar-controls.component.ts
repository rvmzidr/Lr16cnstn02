  ...
  template: `
    ...
    >
      ...
      >
        ...
        @if (!compact) {
          <span class="text-xs font-semibold tracking-[0.2em]">{{
            languageCode()
          }}</span>
        }
      </button>
        ...
        >
          @for (option of primaryLanguages; track option.code) {
            <button
              ...
              [class.toolbar-dropdown__item--active]="
                site.language() === option.code
              "
              (click)="setLanguage(option.code)"
            >
              <span
                class="min-w-8 text-left text-xs font-semibold tracking-[0.2em]"
                >{{ option.code.toUpperCase() }}</span
              >
      ...
      @if (showThemeToggle) {
        <button
          ...
          [attr.aria-label]="
            site.isDarkMode()
              ? site.localize(lightModeLabel)
              : site.localize(darkModeLabel)
          "
          (click)="site.toggleDarkMode()"
        >
          <lucide-icon
            [img]="site.isDarkMode() ? icons.SunMedium : icons.Moon"
            class="h-4 w-4"
          ></lucide-icon>
        </button>
      }
