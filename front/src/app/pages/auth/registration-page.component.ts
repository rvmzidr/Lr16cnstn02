            ...
            @switch (steps[currentStepIndex()].id) {
              @case ('identite') {
                <div class="grid gap-5 md:grid-cols-2">
                  <div><label class="mb-2 block">Nom</label><input [(ngModel)]="form.nom" name="nom" class="input-shell" /></div>
                  <div><label class="mb-2 block">Prenom</label><input [(ngModel)]="form.prenom" name="prenom" class="input-shell" /></div>
                  <div><label class="mb-2 block">Nom de jeune fille</label><input [(ngModel)]="form.nomJeuneFille" name="nomJeuneFille" class="input-shell" /></div>
                  <div><label class="mb-2 block">Sexe</label><select [(ngModel)]="form.sexe" name="sexe" class="select-shell"><option value="FEMME">Feminin</option><option value="HOMME">Masculin</option><option value="AUTRE">Autre</option></select></div>
                  <div><label class="mb-2 block">Date de naissance</label><input [(ngModel)]="form.dateNaissance" name="dateNaissance" type="date" class="input-shell" /></div>
                  <div><label class="mb-2 block">Lieu de naissance</label><input [(ngModel)]="form.lieuNaissance" name="lieuNaissance" class="input-shell" /></div>
                  <div><label class="mb-2 block">CIN tunisien</label><input [(ngModel)]="form.cin" name="cin" class="input-shell" /></div>
                  <div><label class="mb-2 block">Passeport</label><input [(ngModel)]="form.passeport" name="passeport" class="input-shell" /></div>
                </div>
              }
              @case ('contact') {
                <div class="grid gap-5 md:grid-cols-2">
                  <div><label class="mb-2 block">Email institutionnel</label><input [(ngModel)]="form.emailInstitutionnel" name="emailInstitutionnel" type="email" class="input-shell" /></div>
                  <div><label class="mb-2 block">Telephone</label><input [(ngModel)]="form.telephone" name="telephone" class="input-shell" /></div>
                  <div class="md:col-span-2"><label class="mb-2 block">Adresse</label><textarea [(ngModel)]="form.adresse" name="adresse" class="textarea-shell"></textarea></div>
                </div>
              }
              @case ('professionnel') {
                <div class="grid gap-5 md:grid-cols-2">
                  <div><label class="mb-2 block">Grade</label><input [(ngModel)]="form.grade" name="grade" class="input-shell" /></div>
                  <div><label class="mb-2 block">ORCID</label><input [(ngModel)]="form.orcid" name="orcid" class="input-shell" /></div>
                  <div><label class="mb-2 block">Institution d'affectation</label><select [(ngModel)]="form.institutionAffectationId" name="institutionAffectationId" class="select-shell"><option value="">Selectionner</option>@for (item of references()?.institutions || []; track item.id) { <option [ngValue]="item.id">{{ item.nom }}</option> }</select></div>
                  <div><label class="mb-2 block">Equipe de recherche</label><select [(ngModel)]="form.equipeRechercheId" name="equipeRechercheId" class="select-shell"><option value="">Selectionner</option>@for (item of references()?.equipesRecherche || []; track item.id) { <option [ngValue]="item.id">{{ item.nom }}</option> }</select></div>
                  <div><label class="mb-2 block">Dernier diplome obtenu</label><input [(ngModel)]="form.dernierDiplomeLibre" name="dernierDiplomeLibre" class="input-shell" /></div>
                  <div><label class="mb-2 block">Date d'obtention du diplome</label><input [(ngModel)]="form.dateObtentionDiplome" name="dateObtentionDiplome" type="date" class="input-shell" /></div>
                  <div class="md:col-span-2"><label class="mb-2 block">Etablissement du diplome</label><input [(ngModel)]="form.etablissementDiplome" name="etablissementDiplome" class="input-shell" /></div>
                </div>
              }
              @case ('laboratoire') {
                <div class="grid gap-5">
                  <div><label class="mb-2 block">Denomination du laboratoire</label><input [(ngModel)]="form.laboratoireDenomination" name="laboratoireDenomination" class="input-shell" /></div>
                  <div><label class="mb-2 block">Etablissement du laboratoire</label><input [(ngModel)]="form.laboratoireEtablissement" name="laboratoireEtablissement" class="input-shell" /></div>
                  <div><label class="mb-2 block">Universite du laboratoire</label><input [(ngModel)]="form.laboratoireUniversite" name="laboratoireUniversite" class="input-shell" /></div>
                  <div><label class="mb-2 block">Responsable du laboratoire</label><input [(ngModel)]="form.laboratoireResponsable" name="laboratoireResponsable" class="input-shell" /></div>
                </div>
              }
              @case ('doctorat') {
                <div class="space-y-5">
                  ...
                  @if (form.estDoctorant) {
                    <div class="grid gap-5 md:grid-cols-2">
                      <div class="md:col-span-2"><label class="mb-2 block">Sujet de recherche</label><textarea [(ngModel)]="form.sujetRecherche" name="sujetRecherche" class="textarea-shell"></textarea></div>
                      <div><label class="mb-2 block">Pourcentage d'avancement</label><input [(ngModel)]="form.pourcentageAvancement" name="pourcentageAvancement" class="input-shell" /></div>
                      <div><label class="mb-2 block">Annee universitaire de premiere inscription</label><input [(ngModel)]="form.anneeUniversitairePremiereInscription" name="anneeUniversitairePremiereInscription" class="input-shell" placeholder="2025/2026" /></div>
                      <div><label class="mb-2 block">Universite d'inscription</label><input [(ngModel)]="form.universiteInscription" name="universiteInscription" class="input-shell" /></div>
                      <div><label class="mb-2 block">Directeur de these</label><input [(ngModel)]="form.directeurThese" name="directeurThese" class="input-shell" /></div>
                      <div class="md:col-span-2">
                        <label class="mb-2 block">Attestation d'inscription</label>
              ...
              @case ('securite') {
                <div class="grid gap-5 md:grid-cols-2">
                  <div><label class="mb-2 block">Mot de passe</label><input [(ngModel)]="form.motDePasse" name="motDePasse" type="password" class="input-shell" /></div>
                  <div><label class="mb-2 block">Confirmation du mot de passe</label><input [(ngModel)]="form.confirmationMotDePasse" name="confirmationMotDePasse" type="password" class="input-shell" /></div>
                </div>
