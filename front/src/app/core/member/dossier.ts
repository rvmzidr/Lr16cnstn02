...
export type DossierErrorMap = Partial<
  Record<keyof RegistrationPayload | 'attestationDoctorant', string>
>;
...
export function createRegistrationPayloadFromUser(
  ...
  return {
    ...
    institutionAffectationId:
      utilisateur.profil?.institutionAffectationId || '',
    ...
    universiteInscription: blankString(
      utilisateur.doctorat?.universiteInscription,
    ),
...
export function validateRegistrationPayload(
  ...
  if (!payload.grade.trim())
    errors.grade = 'Le grade ou statut est obligatoire.';
  if (!payload.institutionAffectationId) {
    errors.institutionAffectationId =
      "L'etablissement d'affectation est obligatoire.";
  }
  ...
  if (!payload.dateObtentionDiplome) {
    errors.dateObtentionDiplome =
      "La date d'obtention du diplome est obligatoire.";
  }
  ...
  if (payload.estDoctorant) {
    ...
    if (!payload.anneeUniversitairePremiereInscription.trim()) {
      ...
      !isValidAcademicYear(payload.anneeUniversitairePremiereInscription)
    ) {
      errors.anneeUniversitairePremiereInscription =
...
export function appendRegistrationPayloadToFormData(
  ...
  formData.set(
    'institutionAffectationId',
    String(payload.institutionAffectationId || ''),
  );
