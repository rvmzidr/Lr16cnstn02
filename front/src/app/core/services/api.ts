...
async function downloadProtectedFile(path: string, token: string) {
  ...
  const fileName = decodeURIComponent((match?.[1] || "document").replace(/"/g, ""));
...
export const api = {
  ...
  listPublicArticles(query?: RequestOptions["query"]) {
    return request<{ elements: Article[]; meta: PaginationMeta }>("/public/articles", {
      query,
    });
  },
  ...
  listPublicNews(query?: RequestOptions["query"]) {
    return request<{ elements: Actualite[]; meta: PaginationMeta }>("/public/actualites", {
      query,
    });
  },
  ...
  listMemberArticles(token: string) {
    return request<MemberArticlesData>("/membre/articles/mes-articles", { token });
  },
  ...
  addCoAuthor(
    token: string,
    articleId: number,
    payload: { utilisateurId: string; ordreAuteur?: number; auteurCorrespondant?: boolean }
  ) {
  ...
  deleteCoAuthor(token: string, articleId: number, utilisateurId: string) {
    return request<Article>(`/membre/articles/${articleId}/co-auteurs/${utilisateurId}`, {
      method: "DELETE",
      token,
    });
  },
  listMemberNews(token: string, query?: RequestOptions["query"]) {
    return request<{ elements: Actualite[]; meta: PaginationMeta }>("/membre/actualites", {
      token,
      query,
    });
  },
  ...
  validateRegistration(
    ...
    return request<UtilisateurComplet>(`/admin/inscriptions/${userId}/valider`, {
      method: "PATCH",
      token,
      body: payload,
    });
  },
  refuseRegistration(token: string, userId: string, payload: { motifRejet: string }) {
    return request<UtilisateurComplet>(`/admin/inscriptions/${userId}/refuser`, {
      method: "PATCH",
      token,
      body: payload,
    });
  ...
  downloadAdminDoctorantAttestation(token: string, userId: string) {
    return downloadProtectedFile(`/admin/comptes/${userId}/attestation-doctorant`, token);
  },
  ...
  refuseArticle(token: string, articleId: number, payload: { motifRejet: string }) {
